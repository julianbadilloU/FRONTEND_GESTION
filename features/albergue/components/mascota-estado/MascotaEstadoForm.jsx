 ...
 *  - onCancel: () => void — se llama cuando el usuario cancela la operación
 *
 * Maneja internamente: fetch, validación, toast de feedback, modal de confirmación adoptado.
 */
export default function MascotaEstadoForm({ mascotaId, onSuccess, onCancel }) {
    ...
    return (
      <div className="text-center">
        <div className="bg-red-50 text-red-600 p-6 rounded-2xl border border-red-100">
          ...
          {onCancel && (
            <button onClick={onCancel} className="mt-4 text-sm underline">
              Volver atrás
            </button>
          )}
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-5 pb-6 border-b border-[#dcd7ce]">
        <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-white shadow-md bg-gray-200 shrink-0">
          {mascota?.fotos?.[0]?.url_foto ? (
            ...
            />
          ) : mascota?.fotos?.[0] && typeof mascota.fotos[0] === "string" ? (
            <img
              src={mascota.fotos[0]}
              alt={mascota?.nombre}
              className="w-full h-full object-cover"
        ...
        <div className="min-w-0">
          <h2 className="text-2xl font-bold text-gray-900 leading-tight truncate">{mascota?.nombre}</h2>
          <p className="text-gray-500 text-sm font-medium">Ref: {mascota?.codigo || mascotaId}</p>
        </div>
      </div>
