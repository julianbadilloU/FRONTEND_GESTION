 ...
 * opcion_tag: [{ id_opcion, id_tag, valor, estado }]
 *
 * Frontend (UI):
 * id, nombre, tipo, peso, activo, filtro_absoluto, opciones: [{ id, nombre }]
 */
...
export function mapTagFromBackend(tag) {
  ...
  return {
    ...
    // Mapear opcion_tag del backend → opciones del frontend
    opciones: (tag.opcion_tag || []).map((op) => ({
      id: op.id_opcion,
...
export function mapTagToBackend(data) {
  ...
  // Preservar categoria original si viene, sino mapear tipo→categoria
  if (data.categoria !== undefined) {
    backend.categoria = data.categoria;
  } else if (data.tipo !== undefined) {
    backend.categoria = mapTipoToCategoria(data.tipo);
  }
